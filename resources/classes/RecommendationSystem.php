...
class RecommendationSystem
{
    ...
    public function __construct(PDO $pdo, int $program_id)
    {
        $this->pdo = $pdo;
    ...
    /**
     * Score: fewer relevant transactions → higher score.
     * If beneficiary has a transaction matching the program type's relevant purposes, score is lower.
     * If no relevant transaction, score is higher, but always returns a value.
     */
    private function scoreTransaction(int $beneficiary_id): float
    {
        // 1. Get program type for this program
        $stmt = $this->pdo->prepare("SELECT program_type FROM programs WHERE id = ?");
        $stmt->execute([$this->program_id]);
        $program_type = strtolower($stmt->fetchColumn() ?? '');

        // 2. Map program_type to relevant request_purpose values
        $purposeMap = [
            'medical' => ['medical expense'],
            'educational' => ['educational'],
            'financial' => ['burial'], // Example: adjust as needed
            'social' => [], // Add relevant purposes if any
        ];
        $relevantPurposes = $purposeMap[$program_type] ?? [];

        // 3. Count all transactions for this beneficiary
        $stmt = $this->pdo->prepare(
        ...
        // 4. Count relevant transactions for this beneficiary
        if (!empty($relevantPurposes)) {
            $in = str_repeat('?,', count($relevantPurposes) - 1) . '?';
            $params = array_merge([$beneficiary_id], $relevantPurposes);
            $stmt = $this->pdo->prepare(
                "SELECT COUNT(*) FROM transactions WHERE beneficiary_id = ? AND request_purpose IN ($in)"
            );
            $stmt->execute($params);
            $relevantCount = (int)$stmt->fetchColumn();
        } else {
            $relevantCount = 0;
        }

        // 5. Scoring logic:
        // - If no transactions at all: score = 1 (highest)
        // - If has relevant transaction(s): score = 0.3 (lowest)
        // - If has transactions but none relevant: score = 0.6 (medium)
        // - If many transactions (of any type), reduce score further
        if ($totalCount === 0) {
            return 1.0;
        } elseif ($relevantCount > 0) {
            // Lower score if already received relevant help
            return max(0.3, 1 - ($relevantCount / 5));
        } else {
            // Has transactions, but not for this program type
            return max(0.6, 1 - ($totalCount / 10));
        }
    }
